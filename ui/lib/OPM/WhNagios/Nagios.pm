...
# Return global status for a server
sub details_server {
    my $self = shift;
    my $id_server = shift;

    my $sql = $self->prepare(qq{SELECT state, count(*) as num FROM wh_nagios.list_services()
        WHERE id_server = ?
        GROUP BY state}
    );

    $sql->execute($id_server);
    my $details = $sql->fetchall_arrayref( {} );
    $sql->finish();
    return $details;
}

# Return a single service status
sub details_service {
    my $self = shift;
    my $id_server = shift;
    my $id_service = shift;

    my $sql = $self->prepare(qq{SELECT state FROM wh_nagios.list_services()
        WHERE id_server = ?
        AND id = ? }
    );

    $sql->execute($id_server, $id_service);
    my $title = $sql->fetchrow();
    $sql->finish();
    return $title;
}
...
sub service {
    ...
    $sql = $dbh->prepare(
        "SELECT id_metric, label, unit, min, max, critical, warning
          FROM wh_nagios.list_metrics( ? )
          ORDER BY label ;"
    );
    $sql->execute($service_id);
...
sub _delete_label {
    ...
    # Generate comma separated numeric id values,
    # as we are calling a function with VARIADIC argument
    my $id_metrics = join( ',', grep { $_ =~ '^\d+$' } @{$ids} );
